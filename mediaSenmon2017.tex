...
%\maketitle
\begin{center}
 \Large 情報メディア専門ユニットI(演習) (2017 年度版)
\end{center}
\section{全般的な注意}
...
\begin{enumerate}
 ...
 \item 授業はくじを引いて4～5人のグループに分けて議論を行う。
 \item 後半の授業でその日の課題について発表を行う。グループから2名程度と
			 する。４限になったら発表の準備を行うこと。
 \item ２回の授業でグループ全員の発表を行うので。その後、らグループ替えを行う。
 \item ネットにある予習のビデオを見て理解しておくこと
 \item 授業内では与えられた課題をグループごとに行った議論については詳細
			 なメモをとり、その日の発表に生かすこと。
 \item ノートは授業終了時に提出のこと
 \item 作成したファイルのリストの扱いは今後考える
 \item 提出したノートは2，3日のうちに返却する。返却方法は未定
 \item 授業の連絡にメーリングリストを立ち上げる。sで始まるメールアドレス
			 を登録するので、すぐに読めるようにすること。
% \item        \texttt{hilano@ic.kanagawa-it.ac.jp} とする。
\end{enumerate}

この授業で使用するブラウザは Opera とする。
Internet Explorer、Firefox、 Google Chrome でもよいがブラウザによっては
機能が対応していないこともある。各種ブラウザの表示の違いなどを
確認しておくのもよいであろう。
...
\end{Verbatim}
 \item いくつかの画像をまとめて移動させたいときは親要素として
       \texttt{<g>}を追加するとよい。
 \item 不透明度(\REF{39})についても理解すること
\end{itemize}
\subsection{作業内容}
今回から作業内容を増やしました。すべてを行うことを目標としていませんので、
興味を持った課題を選ぶこと。%次の事柄からいくつか選択して作業すること。
\begin{enumerate}
...
\end{itemize}
 ...
 \item テキストにある錯視図形の問題を
       ...
       \begin{itemize}
        ...
        \item 問題4.16の図を作成する。
       \end{itemize}
 \item {\bfseries (ちょっと面倒)}グラデーションにアニメーションをつける(ビデオ3)図形を
 ...
 \item \texttt{polyline}または\texttt{polygon}を用いた図形を描くこと。た
   ...
   \begin{itemize}
    \item Excelなど他のプログラムを用いて正6角形の頂点の座標を計算し、そ
          の結果を用いて正6角形を描く
    \item 一つの\ELM{path}で二つの長方形を描く\InVideo
       \FIGN{0.4}{svg-path-rect2}
    \item 穴が開いた正方形を一つの\ELM{path}で描く\InVideo
...
なお、これらの図形には必要であればグラデーションやアニメーションをつける
 ...
 \begin{itemize}
  \item 今までに作成したSVG画像で繰り返しの部分があるものをパターンで書
        き直し、コードの長さ、変更の手間などの比較、考察をする\InVideo
  \item ザビニの錯視\REF{68}や輝くヘルマン格子、モーガンのねじれのひも
        \REF{70}をパターンを用いて作成する
        \InVideo
  \item パターンを構成する要素の一部の属性にアニメーションをつける。錯視
        図形の場合にはその見え方の変化を報告する\InVideo
 \end{itemize}
 \item 文字列を表示すること
\begin{itemize}
 \item 自分の名前を表示\InVideo
